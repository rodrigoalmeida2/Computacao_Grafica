...
# Função para calcular um ponto da curva de Bézier de grau 2 (Quadrática)
...
def bresenham(x0, y0, x1, y1, canvas):
    ...
    while True:
        canvas.create_rectangle(x0 * 20, y0 * 20, (x0 + 1) * 20, (y0 + 1) * 20, fill="green", outline="black")
        if x0 == x1 and y0 == y1:
            break
        e2 = 2 * err
        if e2 > -dy:
...
# Função para desenhar a curva de Bézier de grau 2
def draw_bezier2():
    try:
        ...
        for t in range(101):
            t = t / 100
            x, y = bezier2(t, (x0, y0), (x1, y1), (x2, y2))
...
# Interface gráfica
root = tk.Tk()
root.title("Curva de Bézier Grau 2")
...
tk.Label(root, text="Ponto Final X2:").grid(row=2, column=0)
entry_x2 = tk.Entry(root)
entry_x2.grid(row=2, column=1)

tk.Label(root, text="Ponto Final Y2:").grid(row=2, column=2)
entry_y2 = tk.Entry(root)
entry_y2.grid(row=2, column=3)

# Botão para desenhar a curva Bézier de grau 2
draw_bezier2_button = tk.Button(root, text="Desenhar Bézier Grau 2", command=draw_bezier2)
draw_bezier2_button.grid(row=3, column=0, columnspan=2)
